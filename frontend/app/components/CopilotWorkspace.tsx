...
export function CopilotWorkspace() {
  ...
  useEffect(() => {
    setToken(localStorage.getItem("career_copilot_token"));
  ...
  const login = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      localStorage.setItem("career_copilot_token", result.access_token);
  ...
  const saveAnswer = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setMessage(null);
    try {
      const result = await request<Interview>(`/interviews/${interview.id}/responses`, { method: "POST", body: JSON.stringify({ question_index: 0, answer }) });
      setInterview(result);
  ...
  return <section className="copilot-shell">
    <aside className="workspace-rail">
      ...
      <div className="rail-footer"><p>{user.display_name}</p><button className="quiet-button" onClick={() => { localStorage.removeItem("career_copilot_token"); setToken(null); setUser(null); setMessage(null); }}>Sign out</button></div>
    </aside>
    <main className="workspace-main">
      <div className="mobile-toolbar"><button className="brand mobile-brand" onClick={() => setView("Overview")} aria-label="Open overview"><span className="mark"><span className="mark-fallback" /></span><span><strong>AI Resume</strong><small>Career Copilot</small></span></button><button className="mobile-signout" onClick={() => { localStorage.removeItem("career_copilot_token"); setToken(null); setUser(null); setMessage(null); }}>Sign out</button></div>
      ...
      {view === "Resumes" && <ResumeUploadPanel accessToken={token} onUploaded={() => void refreshWorkspace()} />}
      ...
      {view === "Roadmap" && <section className="workspace-panel"><div className="panel-intro"><p className="eyebrow">Skill gap → proof</p><h2>Turn missing evidence into a sequence.</h2><p>Roadmaps and portfolio prompts follow the prioritized gaps in the last saved match. They are templates to adapt, not AI-generated claims about your readiness.</p></div>{plan ? <PlanCards plan={plan} /> : <div className="empty-evidence"><span>04</span><p>Run a match, then create its roadmap to see skill-specific steps and portfolio prompts here.</p></div>}</section>}
...
function PlanCards({ plan }: { plan: Plan }) {
  const storageKey = `career_copilot_plan_${plan.roadmap.map((item) => item.skill).join("_")}`;
